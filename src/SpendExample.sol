...
contract SpendExample is ERC721Enumerable, Ownable {
    ...
    */
    ...
    /// @notice spend (burn) 10 ERC20 tokens to mint one NFT
    function spendBurn() external {
        erc20.burn(msg.sender, REDEEM_COST);
        _safeMint(msg.sender, nextTokenId++);
    }

    /// @notice spend (transfer to this contract) 10 ERC20 tokens to mint one NFT
    function spendTransfer() external {
        erc20.transferFrom(msg.sender, address(this), REDEEM_COST);
        _safeMint(msg.sender, nextTokenId++);
    }

    /// @notice Admin can mint NFTs directly
    function adminMint(address to) external onlyOwner {
        _safeMint(to, nextTokenId++);
    }

    /// @notice Admin can withdraw ERC20 tokens
